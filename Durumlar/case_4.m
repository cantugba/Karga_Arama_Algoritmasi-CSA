function [bestSolution, bestFitness, iteration]=case_4(fhd, dimension, maxIteration, fNumber)
...
for t=1:tmax
    ...
    for i=1:N % Update position and memory
        if xnew(i,:)>=l & xnew(i,:)<=u
            ...
            % yarýsý fdb pop diðer yarýsý artn dik ucgen
            if rand < 0.5
                if FdbPop( mem, fit_mem, i, xnew(i,:), ft(i)  )

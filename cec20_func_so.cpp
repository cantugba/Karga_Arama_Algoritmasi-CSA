...
//     printf("\n Input func_num %d Running fun num%d  \n", func_num0, func_num);
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "cec2020-so/input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "cec2020-so/input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		{
			sprintf(FileName, "cec2020-so/input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
